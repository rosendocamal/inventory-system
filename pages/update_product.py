...
st.set_page_config(
    page_title="Actualizar stock producto", page_icon=":material/update:"
)
    ...
    if submitted:
        ...
        with col3:
            ...
            if product["status"] is True:
                ...
                with col4:
                    time.sleep(0.2)
                    product_was_saved: dict[str, bool | str] = inventory.update_stock(
                        code, quantity
                    )
                    ...
                    for percent_complete in range(5):
                        ...
                        if product_was_saved["status"] is True:
                            st.success(
                                product_was_saved["message"], icon=None, width="stretch"
                            )
                            product_df = pd.DataFrame(
                                {
